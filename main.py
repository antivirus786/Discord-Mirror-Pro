...
async def clone_server():
    ...
    if guild_from is None:
        print(f"> Error: Could not find source guild with ID {INPUT_GUILD_ID}")
        print("> Make sure the account is a member of that server")
        return

    if guild_to is None:
        print(f"> Error: Could not find destination guild with ID {GUILD}")
        print("> Make sure the account is a member of that server")
        return
